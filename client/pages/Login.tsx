...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      try {
        const adminRes = await apiPost<{ requiresCode?: boolean }>('/api/admin/login', {
        ...
        if (!/401|Invalid credentials/i.test(msg)) throw adminErr;
      }
  ...
  const handleCodeSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const res = await apiPost<{ token?: string }>('/api/admin/verify', {
        email: formData.email,
        code
      });
      if (res?.token) {
